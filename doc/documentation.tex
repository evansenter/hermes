...
\begin{itemize}
  \optionListItem{i}{The RNA sequence \s to be used by \fft.}
  \optionListItem{j}{The first structure \A to be used by \fft. Any base pairs in \A incompatible with \s (i.e. not a Watson-Crick or GU wobble) are ignored by \fft.}
  \optionListItem{k}{The second structure \B to be used by \fft. The same restrictions apply as with \optionFlag{j}.}
