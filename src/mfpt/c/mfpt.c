...
int main(int argc, char** argv) {
  ...
  klp_matrix = klp_matrix_from_file(parameters.input_file, parameters.input && !parameters.energy_based, &mfpt_usage);
  ...
  if (RUN_TYPE(parameters, TRANSITION_INPUT_FLAG)) {
    ...
    transition_matrix = transition_matrix_from_klp_matrix(&klp_matrix, MATRIX_TYPE(parameters));
  } else {
  ...
  print_transition_matrix_with_klp_positions(klp_matrix, transition_matrix, parameters);
#endif
