...
int estimate_starting_index_to_scan_for_equilibrium(int num_points, const EIGENSYSTEM eigensystem, const POPULATION_PARAMS parameters) {
  ...
  if (fabs(str_1_start - str_1_end) > epsilon || fabs(str_2_start - str_2_end) > epsilon) {
    do {
      ...
      starting_index = i++;
    } while (i < num_points - parameters.window_size && (fabs(str_1_current - str_1_end) > epsilon || fabs(str_2_current - str_2_end) > epsilon));
  }
...
#ifdef INSANE_DEBUG
    printf("%d\t%d\t%d\t%f\t%f\t%e\n", eigensystem_index, window_start, window_start + i, current_proportion, future_proportion, fabs(current_proportion - future_proportion));
#endif
