...
double gsl_equilibrium_root_solver(double logtime, void* params) {
  EQUILIBRIUM_SOLVER_PARAMS* solver_params = (EQUILIBRIUM_SOLVER_PARAMS*)params;
  EIGENSYSTEM eigensystem                  = solver_params->eigensystem;
  POPULATION_PARAMS parameters             = solver_params->parameters;
  int i;
  double result = -(solver_params->boltzmann_probability);

  for (i = 0; i < eigensystem.length; ++i) {
    result +=
      E_ROW_ORDER(eigensystem.vectors, parameters.end_index, i, eigensystem.length) *
      E_ROW_ORDER(eigensystem.inverse_vectors, i, parameters.start_index, eigensystem.length) *
      exp(eigensystem.values[i] * pow(10, logtime));
  }
...
double compute_equilibrium(const EIGENSYSTEM eigensystem, const POPULATION_PARAMS parameters) {
  ...
  double x_lo = -15, x_hi = 15;
  gsl_function F;
  EQUILIBRIUM_SOLVER_PARAMS params = {eigensystem, parameters, boltzmann_probability(parameters)};

  F.function = &gsl_equilibrium_root_solver;
  F.params = &params;
